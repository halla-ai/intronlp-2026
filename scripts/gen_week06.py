#!/usr/bin/env python3
"""Generate notebooks/week-06.ipynb and execute it in a fresh venv to verify."""
...
NB_PATH = ROOT / "notebooks" / "week-06.ipynb"

# toy corpus (examples) - Jeju-themed sentences
SENTENCES = [
    "제주 바다가 아주 푸르다",
    "가을 하늘이 아주 푸르다",
    "우도 바다가 더 맑다",
...
sentences_src = json.dumps(SENTENCES, ensure_ascii=False, indent=4).replace("\n]", "    ]")

cells = [
    md("[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)]"
       "(https://colab.research.google.com/github/halla-ai/intronlp-2026/blob/main/notebooks/week-06.ipynb)\n"
       "\n"
       "# 6주차 실습: 단어를 벡터로 - 동시발생 행렬과 코사인 유사도\n"
       "\n"
       "**목표.** 작은 말뭉치에서 단어가 함께 다니는 단어를 세어 행렬을 만들고, "
       "행을 단어 벡터로 써서 두 단어가 얼마나 비슷한지 숫자로 재 본다."),
    md("## 0. 준비\n"
       "\n"
       "표를 보여 줄 pandas를 설치합니다. Colab에는 이미 설치되어 있어 바로 넘어갑니다.\n"
       "\n"
       "설치 셀에서 오류가 나면 다음 셀로 넘어가지 말고 오류의 마지막 줄을 확인하세요."),
    code("%pip -q install \"pandas>=1.5.3\""),
    code("import sys\n"
         "import math\n"
         "import pandas\n"
         "\n"
    ...
    md("## 1. 먼저 그냥 실행해 보기\n"
       ...
       "\n"
       "이 노트북의 문장은 **설명용 예시**입니다. 제주 이야기 문장 13개로 작은 말뭉치를 직접 만듭니다. "
       "실제 방언 자료(AI Hub 한국어 방언 발화, 제주도)는 각자 승인을 받아야 하고 저장소에 올릴 수 없어서, "
       "1-6에서는 방언 표본도 직접 만든 문장으로 봅니다."),
    md("### 1-1. 제주 이야기 문장 13개\n"
       "\n"
       "바다, 하늘, 귤, 음식에 대한 문장 13개입니다. 단어가 겹치는 자리를 눈여겨보세요. "
       "`푸르다` 는 바다와 하늘 문장에, `먹었다` 는 음식 문장에 나옵니다."),
    code("sentences = " + sentences_src + "\n"
         "\n"
         ...
         "print(\"\\n전체 단어:\", len(vocab), \"개\")"),
    md("### 1-2. 창(window)으로 문맥 보기\n"
       "\n"
       "분포 가설은 이렇게 말합니다. **단어의 뜻은 그 단어가 함께 다니는 단어에 있다.** "
       "함께 다닌다는 것을 재는 방법이 창(window)입니다. 기준 단어에서 왼쪽 오른쪽으로 몇 칸을 볼지 정합니다.\n"
       "\n"
       "아래 셀은 기준 단어 주위를 창으로 훑어 이웃 단어를 보여 줍니다. 창 1은 바로 옆 한 칸, 창 2는 옆 두 칸입니다. "
       "창을 넓히면 이웃이 늘어나는지 확인하세요."),
    code("def show_window(sentence, word, window):\n"
         "    ws = sentence.split()\n"
         "    i = ws.index(word)\n"
         "    lo = max(0, i - window)\n"
    ...
    md("### 1-3. 창을 세면 행렬이 된다\n"
       "\n"
       "이웃을 눈으로만 보지 않고 **세어서 표**로 만듭니다. 두 단어가 한 문장에서 창 안에 함께 들어온 횟수를 칸에 씁니다. "
       "이 표가 **동시발생 행렬**입니다. 3주차에 문장을 세었듯이, 이번 주는 단어 쌍을 셉니다.\n"
       "\n"
       "행렬의 한 행이 단어 하나의 기록입니다. `귤을` 의 행을 봅니다. 귤을과 창 안에서 함께 나온 단어 넷이 기록되어 있습니다. "
       "나머지 38칸은 모두 0입니다. 단어 42개 중 넷과만 함께 다녔기 때문입니다.\n"
       "\n"
       "마지막 표는 42 x 42 행렬에서 단어 여덟 개만 잘라 본 것입니다. 표는 대각선을 기준으로 좌우가 같습니다. "
       "`바다가` 와 `푸르다` 가 함께 나왔다면 `푸르다` 와 `바다가` 도 함께 나온 것이기 때문입니다."),
    code("def build_matrix(sentences, window=2):\n"
         "    vocab = sorted({w for s in sentences for w in s.split()})\n"
    ...
    md("### 1-4. 행의 한 줄이 단어 벡터다\n"
       "\n"
       "행렬의 한 행을 **단어 벡터**로 씁니다. 좌표가 단어 개수만큼 있는 숫자 목록이고, 대부분이 0인 희소한 벡터입니다. "
       "이제 단어를 눈으로 비교하는 대신 **숫자 목록을 비교**할 수 있습니다.\n"
       "\n"
       "두 단어의 벡터에서 **겹치는 좌표**를 봅니다. 귤을과 귤은은 사람에게는 같은 과일이지만 겹치는 좌표가 하나도 없습니다. "
       "바다가와 하늘이는 겹치는 좌표가 있습니다. 이 차이가 다음 단계의 유사도에 그대로 드러납니다."),
    code("pairs = [\"귤을\", \"귤은\", \"감귤\", \"바다가\", \"하늘이\"]\n"
         "for w in pairs:\n"
    ...
    md("### 1-5. 코사인 유사도로 재기\n"
       "\n"
       "벡터 두 개가 얼마나 같은 방향을 보는지 재는 값이 **코사인 유사도**입니다. 같은 방향이면 1, 직각이면 0, 반대 방향이면 -1입니다. "
       "두 단어의 벡터가 겹치는 좌표를 많이 가질수록 값이 1에 가까워집니다.\n"
       "\n"
       "먼저 좌표 두 개짜리 벡터로 감을 잡고, 단어 벡터에 적용합니다. 마지막 표가 이 노트북의 결론입니다. "
       "**사람에게 비슷한 단어가 숫자로도 비슷하게 나오는 경우와, 그렇지 않은 경우가 같은 말뭉치 안에 함께 있습니다.**\n"
       "\n"
       "맨 마지막 목록은 말뭉치 전체에서 유사도가 가장 높은 쌍입니다. `맑다` 와 `우도` 가 1.00으로 나오는데, 뜻이 같아서가 아닙니다. "
       "\"우도 바다가 더 맑다\" 한 문장에서 두 단어의 이웃이 똑같이 `바다가` 와 `더` 이기 때문입니다. **문장이 적으면 한 문장이 유사도를 통째로 정합니다.**"),
    code("def cosine(a, b):\n"
         ...
         "\n"
         "print(\"좌표 2개짜리 예시 벡터\")\n"
         "print(\"[2, 1] ~ [1, 2]:\", round(cosine({0: 2, 1: 1}, {0: 1, 1: 2}), 2), \"(같은 방향쯤)\")\n"
         "print(\"[2, 1] ~ [-1, 2]:\", round(cosine({0: 2, 1: 1}, {0: -1, 1: 2}), 2), \"(직각)\")\n"
         "print(\"[2, 1] ~ [-2, -1]:\", round(cosine({0: 2, 1: 1}, {0: -2, 1: -1}), 2), \"(반대)\")\n"
         "\n"
         "print()\n"
         "print(\"단어 벡터의 유사도\")\n"
         "word_pairs = [\n"
         ...
         "for s, a, b in best[:3]:\n"
         "    print(f\"  {a} ~ {b}: {s:.2f}\")"),
    md("### 1-6. 같은 뜻, 다른 표기: 방언과 표준어\n"
       "\n"
       "제주어 `바당` 은 표준어 `바다` 입니다. 사람에게는 같은 뜻이지만, 동시발생 행렬은 두 단어를 다른 칸으로 셉니다. "
       "두 단어가 비슷하게 나오려면 **이웃이 겹쳐야** 합니다.\n"
       "\n"
       "아래는 직접 만든 설명용 표본입니다. 방언 문장과 표준어 문장을 한 쌍씩 두고, 주변 말이 얼마나 방언인지에 따라 "
       "`바당에서` 와 `바다에서` 의 유사도가 어떻게 달라지는지 봅니다. 어휘는 `하영` (많이), `놀멍` (놀며), `쉬멍` (쉬며)입니다."),
    code("dialect_cases = {\n"
         ...
         "print()\n"
         "print(f\"여섯 문장을 한 말뭉치에 섞으면: 바당에서 ~ 바다에서 = {mixed:.2f}\")"),
    md("## 2. 한 지점만 바꿔 보기\n"
       "\n"
       "아래 셀의 `TODO` 로 표시된 **한 곳만** 바꾸고 다시 실행하세요.\n"
       "\n"
       "> 바꾸기 전 결과를 먼저 확인해 두면 무엇이 달라졌는지 비교할 수 있습니다."),
    md("창 크기를 1에서 3으로 바꿔 봅니다. 창이 좁아지면 이웃이 줄고, 넓어지면 늘어납니다. "
       "행렬의 0이 아닌 칸 수와 단어 유사도가 어떻게 달라지는지 눈여겨보세요.\n"
       "\n"
       "특히 `고기국수를` 과 `갈치조림을` 의 유사도를 지켜보세요. 창 1에서는 두 단어가 이웃을 공유하지 못하지만, "
       "창 2부터는 `먹었다` 를 함께 다니는 단어로 세어 유사도가 올라갑니다."),
    code("# TODO: 창 크기를 바꿔 보세요 (1, 2, 3)\n"
         "window = 2\n"
         "\n"
         "# 아래는 그대로 둡니다\n"
...
with tempfile.TemporaryDirectory(prefix="week06-venv-") as workdir:
    work = Path(workdir)
    venv = work / "venv"
