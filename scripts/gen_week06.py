...
cells = [
    ...
    md("## 1. 먼저 그냥 실행해 보기\n"
       ...
       "\n"
       "이 노트북의 문장은 **설명용 예시**입니다. 관광 빅데이터 같은 진짜 데이터는 로그인이 필요해서 "
       "노트북에 넣을 수 없고, 여기서는 제주 이야기 문장 13개로 작은 말뭉치를 직접 만듭니다."),
    ...
    md("### 1-2. 창(window)으로 문맥 보기\n"
       ...
       "함께 다닌다는 것을 재다는 방법이 창(window)입니다. 기준 단어에서 왼쪽 오른쪽으로 몇 칸을 볼지 정합니다.\n"
       "\n"
    ...
    md("## 3. 확인 질문\n"
       ...
       "2. 1-5 표에서 `바다가` 와 `하늘이` 의 유사도를 옮기고, 1-4에서 본 겹치는 좌표와 연결해 이 값이 어디서 왔는지 설명하세요.\n"
       "3. TODO 셀에서 창을 1로 바꾸면 `고기국수를` 과 `갈치조림을` 의 유사도가 어떻게 되나요? 이유를 창과 이웃 단어로 설명하세요.\n"
